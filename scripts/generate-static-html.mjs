...
async function blogEntries() {
  ...
  const staticFallback = BLOG_SLUGS.map(route => {
    ...
    return a ? { route, title: a.title, description: a.intro, publishedAt: a.publishedAt, content: a.content, faqs: a.faqs, category: a.category, image: a.image } : { route };
  });
  ...
  try {
    ...
    for (const slug of slugs) {
      ...
      let title, description, publishedAt, image, content, faqs, category;
      try {
        const raw = await redis.get(`blog:article:${slug.replace(/^\/blog\//, '')}`);
        const a = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
        title = a?.title; description = a?.intro; publishedAt = a?.publishedAt; image = a?.image; content = a?.content; faqs = a?.faqs; category = a?.category;
      } catch { /* titre par défaut */ }
      entries.push({ route, title, description, publishedAt, image, content, faqs, category });
    }
    return entries;
  } catch {
