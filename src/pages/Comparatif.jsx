...
export default function Comparatif() {
  ...
  const view = c ? (isEn && c.en ? { ...c, ...c.en } : c) : null;

  useEffect(() => {
    if (!view) { document.title = "Comparatif introuvable | simfinly.com"; return; }
    document.title = `${view.title} | simfinly.com`;
    document.querySelector('meta[name="description"]')?.setAttribute("content", view.intro);
    let link = document.querySelector('link[rel="canonical"]');
    if (!link) { link = document.createElement('link'); link.rel = 'canonical'; document.head.appendChild(link); }
    link.href = isEn ? `https://www.simfinly.com/en/comparisons/${slug}` : `https://www.simfinly.com/comparatifs/${slug}`;
  }, [view, slug, isEn]);
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Hanken Grotesk', sans-serif", color: "var(--text)" }}>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 16px 80px" }}>
        <div style={{ padding: "24px 0 8px", fontSize: 12, color: "var(--text-secondary)" }}>
          ...
          {" · "}
          <Link to={isEn ? "/en/comparisons" : "/comparatifs"} style={{ color: "var(--text-secondary)", textDecoration: "none" }}>{isEn ? "Comparisons" : "Comparatifs"}</Link>
          {view && <>{" · "}<span style={{ color: "var(--text)" }}>{view.shortTitle}</span></>}
        </div>
        ...
        {!view ? (
          ...
          <>
            <JsonLd data={{
              ...
              "headline": view.title, "description": view.intro,
              "url": isEn ? `https://www.simfinly.com/en/comparisons/${c.slug}` : `https://www.simfinly.com/comparatifs/${c.slug}`,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14, marginBottom: 28 }}>
              <ProsCons title={view.a.name} pour={view.a.pour} contre={view.a.contre} />
              <ProsCons title={view.b.name} pour={view.b.pour} contre={view.b.contre} />
            </div>

            {/* Tableau comparatif */}
            <div style={{ marginBottom: 28, overflowX: "auto" }}>
              <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 600, color: "var(--text)", marginBottom: 14 }}>{isEn ? "Criterion by criterion" : "Comparaison critère par critère"}</h2>
              <table style={{ width: "100%", borderCollapse: "collapse", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
                <thead>
                  <tr style={{ background: "var(--card-bg)" }}>
                    <th style={{ ...colHead, color: "var(--text-secondary)" }}>{isEn ? "Criterion" : "Critère"}</th>
                    <th style={{ ...colHead, color: "var(--gold)" }}>{view.a.name}</th>
                    <th style={{ ...colHead, color: "var(--gold)" }}>{view.b.name}</th>
                  </tr>
                </thead>
                <tbody>
                  {view.criteres.map((row, i) => (
                    <tr key={i}>
            ...
            <div style={{ background: "rgba(184,147,74,0.1)", border: "1px solid var(--border-gold)", borderRadius: 14, padding: "20px 22px", marginBottom: 28 }}>
              <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 19, fontWeight: 700, color: "var(--gold)", marginBottom: 10 }}>{isEn ? "Our verdict" : "Notre verdict"}</h2>
              <p style={{ fontSize: 15, lineHeight: 1.8, color: "var(--text)" }}>{view.verdict}</p>
            </div>
            ...
            {terms.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 600, color: "var(--text)", marginBottom: 14 }}>{isEn ? "Terms to know" : "Les termes à connaître"}</h2>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {terms.map(t => (
                    <Link key={t.slug} to={isEn ? `/en/glossary/${t.slug}` : `/lexique/${t.slug}`} title={isEn ? t.en.short : t.short} style={{
            ...
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: 24 }}>
              <Link to={isEn ? "/en/comparisons" : "/comparatifs"} style={{ fontSize: 13, color: "var(--text-secondary)", textDecoration: "none" }}>{isEn ? "← All comparisons" : "← Tous les comparatifs"}</Link>
            </div>
          </>
        )}
      </div>
