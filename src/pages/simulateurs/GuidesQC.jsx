...
import { GUIDES_QC } from "../../data/guidesQC.js";
...
export default function GuidesQC() {
  const [theme, setTheme] = useTheme();

  useEffect(() => {
    document.title = "Guides financiers québécois | simfinly.com";
    document.querySelector('meta[name="description"]')?.setAttribute("content",
      "Guides pratiques sur les finances personnelles au Québec : rente RRQ, impôt provincial et fédéral, disposition réputée au décès, REER et CELI.");
    let link = document.querySelector('link[rel="canonical"]');
    if (!link) { link = document.createElement('link'); link.rel = 'canonical'; document.head.appendChild(link); }
    link.href = 'https://www.simfinly.com/qc/guides';
  }, []);
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "clamp(26px, 4vw, 38px)", fontWeight: 700, color: "var(--text)", marginBottom: 12, lineHeight: 1.2 }}>
            Guides financiers — <span style={{ color: "var(--primary)" }}>Québec</span>
          </h1>
          <p style={{ fontSize: 16, color: "var(--text-secondary)", lineHeight: 1.7, maxWidth: 620, margin: 0 }}>
            Des guides pratiques sur les grandes thématiques financières québécoises : rente RRQ, impôt combiné Québec/fédéral, succession et épargne-retraite.
          </p>
        </div>
