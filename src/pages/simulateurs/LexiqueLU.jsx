...
import { GLOSSAIRE_LU } from "../../data/glossaireLU.js";
...
export default function LexiqueLU() {
  const [theme, setTheme] = useTheme();
  const [query, setQuery] = useState("");

  useEffect(() => {
    document.title = "Lexique financier luxembourgeois | simfinly.com";
    document.querySelector('meta[name="description"]')?.setAttribute("content",
      "Lexique des termes financiers luxembourgeois : CNAP, classes d'impôt, quotient conjugal, salaire social minimum, frontalier, totalisation des périodes et plus. Définitions claires.");
    let link = document.querySelector('link[rel="canonical"]');
    if (!link) { link = document.createElement('link'); link.rel = 'canonical'; document.head.appendChild(link); }
    link.href = 'https://www.simfinly.com/lu/lexique';
  }, []);

  const q = norm(query.trim());
  const filtered = useMemo(() => {
    if (!q) return GLOSSAIRE_LU;
    return GLOSSAIRE_LU.filter(t =>
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Hanken Grotesk', sans-serif", color: "var(--text)" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <nav aria-label="Fil d'Ariane" style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 28, display: "flex", gap: 6, alignItems: "center" }}>
          <Link to="/lu" style={{ color: "var(--primary)", textDecoration: "none" }}>Accueil Luxembourg</Link>
        ...
        <div style={{ marginBottom: 36 }}>
          <div style={{
            ...
            {GLOSSAIRE_LU.length} termes financiers luxembourgeois
          </div>
          <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "clamp(26px, 4vw, 38px)", fontWeight: 700, color: "var(--text)", marginBottom: 12, lineHeight: 1.2 }}>
            Lexique financier — <span style={{ color: "var(--primary)" }}>Luxembourg</span>
          </h1>
          <p style={{ fontSize: 16, color: "var(--text-secondary)", lineHeight: 1.7, maxWidth: 580, margin: 0 }}>
            Les termes clés de la fiscalité et des finances personnelles luxembourgeoises, expliqués simplement.
          </p>
        </div>
        ...
        <div style={{ position: "relative", marginBottom: 32 }}>
          ...
          <input
            ...
            placeholder="Rechercher un terme (CNAP, classe d'impôt, frontalier…)"
            aria-label="Rechercher un terme du lexique luxembourgeois"
            style={{
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 28, marginTop: 48 }}>
          <Link to="/lu" style={{ color: "var(--primary)", textDecoration: "none", fontSize: 14, fontWeight: 500, display: "inline-flex", alignItems: "center", gap: 6 }}>
            ← Retour à l'accueil Luxembourg
          </Link>
        </div>
      </div>
